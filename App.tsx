...
import { Reservation, AppView, Announcement, MenuItem } from './types';
import AdminDashboard from './components/AdminDashboard';
import { fetchReservations, createReservation, updateReservationStatusService, checkConnection, fetchAnnouncements, fetchMenu, updateMenuItemPrice, DEFAULT_MENU_ITEMS } from './services/supabase';

// --- Data: Menu Items Categories ---
const CATEGORIES = [
...
const App: React.FC = () => {
  ...
  // Menu State - Initialize with DEFAULTS so it never blinks empty
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Load active announcements (Max 2)
        const allAnnouncements = await fetchAnnouncements();
        const active = allAnnouncements.filter(a => a.isActive).slice(0, 2);
        setActiveAnnouncements(active);

        // Load Menu - Service now guarantees non-empty array (fallback to defaults)
  ...
  const filteredItems = activeCategory === 'destaques' 
    ? menuItems.filter(item => item.highlight)
  ...
  const handleReservationSubmit = async (e: React.FormEvent) => {
    ...
    try {
      showToast("Realizando reserva...", "info");
      ...
      if (newReservation) {
        ...
        showToast("Reserva confirmada com sucesso! Te esperamos lá.");
  ...
  const updateMenuPrice = async (id: string, newPrice: number) => {
    try {
      const updated = await updateMenuItemPrice(id, newPrice);
    ...
    return (
      <>
        <AdminDashboard 
          ...
          onUpdateMenuPrice={updateMenuPrice}
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-200 font-sans selection:bg-orange-600 selection:text-white pb-20">
      ...
      {/* GLOBAL ANNOUNCEMENT STACK - Fixed Below Navbar (Top 14 = 56px) */}
      <div className="fixed top-14 left-0 w-full z-40 flex flex-col pointer-events-none">
        {activeAnnouncements.map((ann, index) => (
          <div 
            key={ann.id}
            className="w-full bg-orange-600 text-white py-2 px-4 shadow-lg flex justify-center items-center text-center animate-in slide-in-from-top-4 duration-500 border-b border-orange-700 pointer-events-auto relative"
            style={{ animationDelay: `${index * 100}ms` }}
          >
            <div className="flex items-center gap-2 max-w-7xl mx-auto">
              <Megaphone size={16} className="fill-white/20" />
              <p className="font-bold text-xs md:text-sm tracking-wide uppercase truncate max-w-[90vw]">
                {ann.message}
              </p>
            </div>
          </div>
      ...
      {/* 1. NAVBAR - THINNER VERSION (h-14) */}
      <nav className="fixed w-full z-50 transition-all duration-300 bg-stone-950/80 backdrop-blur-md border-b border-white/5 top-0 h-14">
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          >
            <div className="relative">
               ...
               <Flame className={`text-orange-500 relative z-10 transition-transform ${logoClicks > 0 ? 'scale-110' : ''}`} size={24} fill="currentColor" />
            </div>
            <div className="flex flex-col">
              <span className="font-serif text-lg font-bold tracking-widest text-white leading-none">FUEGO</span>
              <span className="text-[0.5rem] uppercase tracking-[0.3em] text-orange-500 font-medium">Prime Steakhouse</span>
            </div>
          </div>
          
          {/* Desktop Navigation */}
          <div className="hidden md:flex gap-8 text-xs font-medium tracking-wider text-stone-400 items-center">
          ...
          <button onClick={() => { document.getElementById('reservas')?.scrollIntoView({ behavior: 'smooth' }) }} className="hidden md:block bg-orange-600 text-white px-5 py-1.5 rounded-full text-[10px] font-bold tracking-widest hover:bg-orange-700 transition-all shadow-lg shadow-orange-900/20">
            RESERVAR
          </button>
      ...
      <section id="menu" className="py-24 bg-stone-950 relative">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          {filteredItems.length === 0 ? (
            <div className="w-full bg-stone-900 border border-stone-800 rounded-2xl p-12 text-center animate-in fade-in">
              <div className="bg-stone-800 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4">
                <AlertTriangle size={32} className="text-stone-500" />
              </div>
              <h3 className="text-white font-bold text-xl mb-2">Nenhum item encontrado</h3>
              <p className="text-stone-500 max-w-md mx-auto">
                Não há itens cadastrados nesta categoria no momento. Tente selecionar outra opção ou verifique se o banco de dados está populado.
              </p>
            </div>
          ) : (
